...
//     // Get blog by slug (slug must be URL-encoded in usage)
//     getBlogByID: builder.query<{ message: string; data: responseBlog }, string>({
//       query: (slug) => `/post/${slug}`, // ✅ encodeURIComponent(slug) before calling this
//       providesTags: (result, error, slug) => [{ type: "Blog", id: slug }],
//     }),
...
//       { message: string; data: { blogs: responseBlog[]; totalBlog: number } },
//       string
//     >({
//       query: (userId: string) => `/user/blogs/${userId}/`,
//     }),
...
export const blogsApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    // Get blog by slug using a query parameter
    getBlogByID: builder.query<{ message: string; data: responseBlog }, string>({
      // CORRECTED: This now accepts a query string like "?slug=my-first-post"
      // This aligns with the unified API route on the backend.
      query: (queryString) => `/post${queryString}`,
      providesTags: (result, error, slug) => [{ type: "Blog", id: slug }],
    }),
